...
__kernel void sgemm(const __global float* a,
    ...
    for(int i = 0; i < tile_num; ++i){
        ...
        for(int j = 0; j < 4; ++j){
            ...
            *(a_block + (j + local_col) * TS + local_row) = *(a + (tile_col + j) * m + global_row);
            *(b_block + (j + local_col) * TS + local_row) = *(b + tile_row + j + global_col * k);
        }
        ...
        for(int t = 0; t < TS; ++t){
            for(int j = 0; j < 4; ++j){
                acc[j] += (*(a_block + t * TS + local_row)) * (*(b_block + t + (local_col + j) * TS));
            }
        }
    ...
    for(int j = 0; j < 4; ++j){
        *(c + (global_col + j) * m + global_row) = acc[j];
    }
}
